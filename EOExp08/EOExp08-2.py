...
filenames = [
    [os.path.join('Exp08_Data', f'flash_light{i:01d}.txt') for i in range(1, 2)],
    [os.path.join('Exp08_Data', f'no_light{i:01d}.txt')    for i in range(1, 2)],
    [os.path.join('Exp08_Data', f'red_laser{i:01d}.txt')   for i in range(1, 2)],
]
...
for i in range(len(filenames)):
    ...
    for j in range(len(filenames[i])):
        ...
        axes[i].plot(x_values[i][j], y_values[i][j], label=legends[i][0], color=colors[i*4+j])
    ...
    figs[i].savefig(os.path.join(working_dir, f'fig_{legends[i][0]}_single.png'))
